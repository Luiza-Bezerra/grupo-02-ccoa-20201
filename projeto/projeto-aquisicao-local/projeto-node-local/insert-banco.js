...
function registrar_leitura(luminosidade) {
    ...
    banco.conectar().then(() => {
        ...
        INSERT into eventos (idEventos, GrauLum, fkSensor, DataEvento)
        values (
            (select count(*)+1 from eventos),
            ${luminosidade}, 1, CONVERT(Datetime, '${agora()}', 120));
        
        delete from eventos where IdEventos not in 
        (select top ${registros_mantidos_tabela_leitura} IdEventos from eventos order by IdEventos desc);`);
...
if (gerar_dados_aleatorios) {
	...
	setInterval(function() {
		...
		registrar_leitura(parseInt(Math.random()*1024))
